...
from typing import (
    Dict, List, Any, Optional, Union, Tuple, TypedDict, Literal,
    Protocol, cast, TypeVar, Annotated, overload
)

# Add parent directory to the path to import framework
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# Import using the Python 3.12 version of the base component
from framework.base_component_py312 import BaseComponent, ComponentConfig, Artifact
...
try:
    ...
    from boto3.resources.base import ServiceResource as ServiceResourceType
...
# TypedDict definitions for S3 component
class S3Config(ComponentConfig, total=False):
    """TypedDict for S3 component configuration."""
    private_bucket: str
    public_bucket: str
    endpoint: str
    access_key: str
    ...
    create_metadata_index: bool
    cleanup_old_artifacts: bool
    max_artifact_age_days: int


class BucketStatus(TypedDict):
    """TypedDict for bucket status."""
    exists: bool
    objects_count: int
    folders: List[str]


class BucketProcessingStatus(TypedDict, total=False):
    """TypedDict for bucket processing status."""
    created: bool
    configured: bool
...
class DiscoveryResults(TypedDict, total=False):
    """TypedDict for discovery results."""
    connectivity: bool
    bucket_count: int
    endpoint: str
    error: str
...
class ProcessingResults(TypedDict, total=False):
    """TypedDict for processing results."""
    actions: List[str]
    buckets: Dict[str, BucketProcessingStatus]


class HousekeepingResults(TypedDict, total=False):
    """TypedDict for housekeeping results."""
    verification: Dict[str, bool]
    metadata_index: Dict[str, Union[bool, int]]
    warnings: List[str]
    cleanup: Dict[str, Any]


class MetadataEntry(TypedDict, total=False):
    """TypedDict for metadata entry."""
    key: str
    size: int
...
class S3Component(BaseComponent):
    ...
    def __init__(self, config: S3Config, logger: Optional[logging.Logger] = None) -> None:
        """
        ...
        """
        # Merge provided config with defaults using Python 3.12 syntax
        ...
        # Bucket references
        self.private_bucket: Optional[Bucket] = None
        self.public_bucket: Optional[Bucket] = None
    ...
    def _load_env_config(self, config: S3Config) -> None:
        """
    ...
    def discover(self) -> Dict[str, Any]:
        ...
        try:
            # Initialize discovery results with proper type annotation
    ...
    def _check_s3_connectivity(self) -> None:
        ...
        try:
            ...
            # Initialize S3 client and resource - Python 3.12 allows more precise types
            ...
            # Update discovery results with Python 3.12 dict update syntax
            self.discovery_results |= {
                'connectivity': True,
    ...
    def _discover_buckets(self) -> None:
        ...
        """
        if not self.discovery_results.get('connectivity'):
        ...
        if private_exists and self.s3_resource:
            ...
            # Get object count
            try:
                # Python 3.12 improved list comprehension performance
                ...
                # Extract folder structure using set comprehension
                folders = {f"{parts[0]}/" for obj in private_objects 
                          if (parts := obj.key.split('/')) and len(parts) > 1}
                ...
                try:
                    ...
                    self.logger.warning(f"Could not get versioning for private bucket: {ve}")
                ...
                try:
                    ...
                    self.logger.info(f"No policy set for private bucket: {pe}")
        ...
        if public_exists and self.s3_resource:
            ...
            try:
                ...
                # Extract folder structure using set comprehension
                folders = {f"{parts[0]}/" for obj in public_objects 
                          if (parts := obj.key.split('/')) and len(parts) > 1}
                ...
                try:
                    ...
                    self.logger.warning(f"Could not get versioning for public bucket: {ve}")
                ...
                try:
                    ...
                    self.logger.info(f"No policy set for public bucket: {pe}")
    ...
    def process(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            # Initialize processing results with proper type annotation
            ...
            if create_buckets:
                ...
                if (self.discovery_results.get('buckets', {}).get('public', {}).get('exists', False) or 
                    self.processing_results.get('buckets', {}).get('public', {}).get('created', False)):
    ...
    def _setup_bucket(self, bucket_type: Literal["private", "public"], bucket_name: str, 
        ...
        bucket_exists = self.discovery_results.get('buckets', {}).get(bucket_type, {}).get('exists', False)
        ...
        if not bucket_exists:
            ...
            if bucket_type == 'private' and not self.discovery_results.get('versioning', {}).get('private', False):
                try:
                    self.s3_client.put_bucket_versioning(
                        Bucket=bucket_name,
                        VersioningConfiguration={'Status': 'Enabled'}
            ...
            existing_folders = self.discovery_results.get('buckets', {}).get(bucket_type, {}).get('folders', [])
            for folder in folder_structure:
    ...
    def _configure_public_bucket_policy(self) -> None:
        ...
        try:
            ...
            # Use dict update syntax for Python 3.12
            if 'policy_configured' not in self.processing_results['buckets']['public']:
    ...
    def _verify_configurations(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # Python 3.12 improved pattern matching for dictionaries
                has_public_read = False
                for statement in policy_json.get('Statement', []):
                    if (statement.get('Effect') == 'Allow' and 
                        (statement.get('Principal', {}).get('AWS') == '*' or statement.get('Principal') == '*') and
                        ('s3:GetObject' in statement.get('Action', []) or statement.get('Action') == 's3:GetObject')):
    ...
    def _create_metadata_index(self) -> None:
        ...
        """
        if not self.s3_client or not self.private_bucket:
            self.logger.error("S3 client or private bucket not initialized")
            return
        ...
        try:
            ...
            # Extract metadata - Python 3.12 optimized comprehension
            ...
            for obj in objects:
                try:
                    ...
                    # Python 3.12 TypedDict for structured output
                    entry: MetadataEntry = {
                        'key': obj.key,
            ...
            for obj in objects:
                try:
                    ...
                    if last_modified and last_modified.replace(tzinfo=None) < cutoff_date:
                        # Delete the object - Python 3.12 improved string formatting
    ...
    def _store_artifacts(self) -> None:
        ...
        """
        if not self.s3_client:
            self.logger.debug("S3 client not initialized")
            return
    ...
    def upload_iso(self, iso_path: str, server_id: str, hostname: str, 
                   version: str, publish: bool = True) -> UploadResult:
        """
    ...
    def list_isos(self, server_id: Optional[str] = None, 
        ...
        try:
            ...
            objects = list(self.s3_resource.Bucket(private_bucket_name).
