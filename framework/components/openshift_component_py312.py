...
from typing import (
    Dict, Any, Optional, List, Union, TypedDict, Literal, 
    Protocol, cast, overload, Callable
)
...
# TypedDict definitions for OpenShift component
class OpenShiftConfig(ComponentConfig, total=False):
    """TypedDict for OpenShift component configuration."""
...
class S3ComponentProtocol(Protocol):
    """Protocol for S3Component interface."""
    s3_client: Any
    s3_resource: Any
    
    def upload_file(self, file_path: str, bucket: str, key: str, metadata: Dict[str, str]) -> bool:
        ...


class DiscoveryResults(TypedDict, total=False):
    """TypedDict for OpenShift discovery results."""
    installed_versions: List[str]
    available_versions: List[str]
    existing_isos: List[str]
...
class ProcessingResults(TypedDict, total=False):
    """TypedDict for OpenShift processing results."""
    installer_downloaded: bool
    configs_created: bool
    iso_generated: bool
...
class HousekeepingResults(TypedDict, total=False):
    """TypedDict for OpenShift housekeeping results."""
    iso_verified: bool
...
class ISOMetadata(TypedDict, total=False):
    """TypedDict for ISO metadata."""
    version: str
    domain: str
    rendezvous_ip: Optional[str]
    size_bytes: int
...
class OpenShiftComponent(BaseComponent):
    ...
    DEFAULT_CONFIG: OpenShiftConfig = {
    ...
    def __init__(self, config: OpenShiftConfig, logger: Optional[logging.Logger] = None, s3_component: Optional[S3ComponentProtocol] = None):
        """
        ...
        self.s3_component: Optional[S3ComponentProtocol] = s3_component
    ...
    def _discover_openshift_client(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Extract version numbers with improved regex
                import re
                version_matches = re.findall(r'(\d+\.\d+\.\d+)', version_info)
                
                # Use Python 3.12 extend functionality - clearer than append in a loop
                self.discovery_results['available_versions'].extend(version_matches)
    ...
    def _discover_openshift_installer(self) -> None:
        ...
        # Check common paths for openshift-install - using Python 3.12 path handling
        installer_paths = [
            Path.cwd() / "openshift-install",
            Path.cwd() / "bin" / "openshift-install",
            Path.home() / ".local/bin/openshift-install",
            Path("/usr/local/bin/openshift-install")
        ]
        
        # If output directory is specified, check there too
        if output_dir := self.config.get('output_dir'):
            installer_paths.append(Path(output_dir) / "openshift-install")
        
        for path in installer_paths:
            # Use Python 3.12 improved Path methods
            if path.exists() and os.access(path, os.X_OK):
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        # Extract version
                        import re
                        if version_matches := re.findall(r'(\d+\.\d+\.\d+)', version_info):
                            self.discovery_results['installer_available'] = True
                            
                            # Use set operations to avoid duplicates
                            available_versions = set(self.discovery_results.get('available_versions', []))
                            available_versions.update(version_matches)
                            self.discovery_results['available_versions'] = list(available_versions)
                        
                        # Store installer path
                        self.discovery_results['installer_path'] = str(path)
                        break
                except Exception as e:
    ...
    def _discover_existing_isos(self) -> None:
        ...
        # Define common locations to search using Path objects for Python 3.12
        search_paths = [
            Path.cwd() / "isos",
            Path.cwd() / "downloads",
            Path("/tmp")
        ...
        # Search for ISO files with improved Python 3.12 syntax
        found_isos: List[str] = []
        
        for path in search_paths:
            if path.exists():
                # More efficient Path.glob usage in Python 3.12
                for iso_file in path.glob("**/*.iso"):
                    # Check if it's likely an OpenShift ISO
                    iso_name = iso_file.name.lower()
                    if any(keyword in iso_name for keyword in ["openshift", "ocp", "agent"]):
                        found_isos.append(str(iso_file))
                        self.logger.info(f"Found ISO: {iso_file}")
    ...
    def _discover_pull_secret(self) -> None:
        ...
        if pull_secret_path := self.config.get('pull_secret_path'):
            full_path = Path(os.path.expanduser(pull_secret_path))
            if full_path.exists():
                self.logger.info(f"Pull secret found at {full_path}")
                self.discovery_results['pull_secret_available'] = True
                self.discovery_results['pull_secret_path'] = str(full_path)
                return
        
        # Check default path
        default_path = Path.home() / ".openshift/pull-secret"
        if default_path.exists():
            self.logger.info(f"Pull secret found at {default_path}")
            self.discovery_results['pull_secret_available'] = True
            self.discovery_results['pull_secret_path'] = str(default_path)
            return
    ...
    def _discover_ssh_key(self) -> None:
        ...
        # Check specified path if provided
        if ssh_key_path := self.config.get('ssh_key_path'):
            full_path = Path(os.path.expanduser(ssh_key_path))
            if full_path.exists():
                self.logger.info(f"SSH key found at {full_path}")
                self.discovery_results['ssh_key_available'] = True
                self.discovery_results['ssh_key_path'] = str(full_path)
                return
        ...
        if default_path.exists():
            ...
            self.discovery_results['ssh_key_available'] = True
            self.discovery_results['ssh_key_path'] = str(default_path)
            return
    ...
    def _download_installer(self) -> None:
        ...
        # Skip if temp_dir is not defined
        if not self.temp_dir:
            self.logger.error("Temporary directory not set up")
            return
        ...
        if self.discovery_results.get('installer_available') and (existing_installer := self.discovery_results.get('installer_path')):
            try:
        ...
        try:
            ...
            # Determine download URL based on version using Python 3.12 style conditional
            if version == 'stable':
            ...
            # Use import inside try block for better exception handling
            try:
                import requests
            ...
            if os.path.exists(extracted_installer):
                ...
                # If not found at expected path, search for it - improved with Python 3.12 path handling
                download_path = Path(download_dir)
                found = False
                
                # More efficient way to find files in Python 3.12
                for path in download_path.glob("**/openshift-install"):
                    shutil.move(str(path), installer_path)
                    found = True
                    break
    ...
    def _generate_iso(self) -> None:
        ...
        self.processing_results['iso_path'] = os.path.join(self.temp_dir or '', "agent.x86_64.iso")
    ...
    def _upload_to_s3(self) -> None:
        ...
        # Create metadata with Python 3.12 explicit type
        ...
        if self.s3_component:
            try:
                ...
                # Using Python 3.12 pattern matching for cleaner code
                match (server_id, hostname):
    ...
    def _update_metadata(self) -> None:
        ...
        metadata: ISOMetadata = {
            'openshift_version': self.config.get('openshift_version', 'stable'),
            'domain': self.config.get('domain', ''),
            'rendezvous_ip': self.config.get('rendezvous_ip'),
